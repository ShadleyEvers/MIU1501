...
NSString * Evers_Shadley_InfoApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation Evers_Shadley_InfoAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_evers_shadley_infoapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Evers_Shadley_InfoApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Evers_Shadley_InfoApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_evers_shadley_infoapp = [[Evers_Shadley_InfoAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_evers_shadley_infoapp];
	
	NSString *_evers_shadley_infoappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _evers_shadley_infoappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_evers_shadley_infoapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Evers_Shadley_InfoApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.evers_shadley_infoapp.kroll" 
