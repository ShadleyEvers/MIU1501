...
NSString * Evers_Shadley_UtilityApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation Evers_Shadley_UtilityAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_evers_shadley_utilityapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Evers_Shadley_UtilityApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Evers_Shadley_UtilityApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_evers_shadley_utilityapp = [[Evers_Shadley_UtilityAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_evers_shadley_utilityapp];
	
	NSString *_evers_shadley_utilityappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _evers_shadley_utilityappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_evers_shadley_utilityapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Evers_Shadley_UtilityApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.evers_shadley_utilityapp.kroll" 
