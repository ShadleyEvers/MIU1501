...
NSString * Evers_Shadley_AboutMe$ModuleRequireFormat = @"(function(exports){"
...
@implementation Evers_Shadley_AboutMeObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_evers_shadley_aboutme);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Evers_Shadley_AboutMe global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Evers_Shadley_AboutMe" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_evers_shadley_aboutme = [[Evers_Shadley_AboutMeObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_evers_shadley_aboutme];
	
	NSString *_evers_shadley_aboutmeNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _evers_shadley_aboutmeNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_evers_shadley_aboutme valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Evers_Shadley_AboutMe$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.evers_shadley_aboutme.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.evers_shadley_aboutme.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
