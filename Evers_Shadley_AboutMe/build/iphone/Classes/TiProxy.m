...
//Rare exceptions to indicate a bug in the _evers_shadley_aboutme code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_evers_shadley_aboutme = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_evers_shadley_aboutme]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.evers_shadley_aboutme.module"
